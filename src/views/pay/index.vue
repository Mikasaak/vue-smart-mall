...
import { settleOrder, submitOrder } from '@/api/order'
...
export default {
  ...
  methods: {
    ...
    async settleOrder () {
      ...
      const { data: { order, personal } } = await settleOrder(this.mode, obj)
    ...
    async submitOrder () {
      ...
      const { data: { payType, orderId } } = await submitOrder(this.mode, obj)
  ...
  computed: {
    ...
    addressInfo () {
      let addressInfo = ''
      if (this.selectedAddress && this.selectedAddress.region) {
        for (const key in this.selectedAddress.region) {
          addressInfo += this.selectedAddress.region[key] + ' '
        }
        addressInfo += this.selectedAddress.detail
      } else {
        addressInfo = '请选择配送地址'
      }
      return addressInfo
